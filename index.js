...
// Rotas
    ...
    app.get('/deletar/:pk_produto', (req, res) => {
        Produto.destroy({where: {'pk_produto': req.params.pk_produto}}).then(() => {
            res.send(`
            <script>              
    ...
    app.get('/editar/:id', async (req, res) => {
        try {
            ...
            return sendErrorResponse(res, 'Erro ao editar ao ir a tela de edicao de produto!', '/consultaProduto');
        }
    });
    ...
    app.put('/editar/:pk_produto', async (req, res) => { 
        try {
            ...
            const { descProduto, catProduto, valorCompraProduto, valorBrutoVendaProduto, quantidadeProduto } = req.body;
...
app.get('/consultaVenda', async (req, res) => {
    try {
        const venda = await Venda.findAll({  // Adicionando "await" e armazenando o resultado
...
app.get('/deletarVenda/:pk_venda', (req, res) => {
    Venda.destroy({where: {'pk_venda': req.params.pk_venda}}).then(() => {
        res.send(`
        <script>              
...
app.post('/tbl_venda', async (req, res) => {
    try {
        let { dataVenda, descProduto, qntVendaProduto, totalPedidoVenda, descontoVenda, totalPagoVenda, formaPagVenda } = req.body;

        // Buscar o produto pelo nome da descrição
        const produto = await Produto.findOne({ where: { descricao_produto: descProduto } });
